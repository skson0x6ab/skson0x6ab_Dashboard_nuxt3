...
import { useJsonDataStoreStarRail } from '@/stores/dataStoreStarRail'; // Pinia 스토어 임포트

// Pinia 스토어 사용
const dataStore = useJsonDataStoreStarRail();
...
// 각각 다른 filepath 설정
const filepath = 'StarRail.json'; // 이 컴포넌트에서 가져올 파일 경로

// 컴포넌트가 마운트될 때 데이터를 가져옵니다.
onMounted(async () => {
  await dataStore.fetchJsonData(filepath);
  // Pinia 스토어가 데이터 갱신되면 ref에 반영
  data.value = dataStore.jsonDataStarRail;
